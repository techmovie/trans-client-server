...
export default class Deluge {
  ...
  async request (methodName, params = [], method = 'POST', options = {}) {
    ...
    try {
      ...
      if (res.body.error) {
        if (res.body.error.message === 'Not authenticated') {
          this.request(methodName, params, method, options)
        } else {
          throw new Error(res.body.error.message || '请求失败')
        }
      }
      return {
  ...
  async addTorrentFile (params) {
    ...
    if (typeof torrent === 'string') {
      if (fs.existsSync(torrent)) {
        fileData = Buffer.from(fs.readFileSync(torrent)).toString('base64')
      } else {
        fileData = Buffer.from(torrent, 'base64').toString('base64')
      }
    } else {
      fileData = torrent.toString('base64')
    }
    reqParams.filedump = fileData
